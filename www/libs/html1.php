...
function do_header($title, $id='home', $options = false) {
	...
	if ($this_site->sub) {
		// We modify url, not sure if it's actually a bug or a misconfiguration
		$this_site->url = $this_site->base_url.'m/'.$this_site->name;
		// $this_site->url = $this_site->name;
	} else {
	...
	if (! is_array($options)) {
		...
		if ($this_site->enabled && empty($this_site_properties['new_disabled'])) {
			$left_options[] = new MenuOption(_('enviar poema'), $globals['base_url'].'submit', $id, _('enviar nuevo poema'));
		}
		$left_options[] = new MenuOption(_('portada'), $globals['base_url'], $id, _('página principal'));
		$left_options[] = new MenuOption(_('nuevos'), $globals['base_url'].'queue', $id, _('votar poemas pendientes'));
		$left_options[] = new MenuOption(_('populares'), $globals['base_url'].'popular', $id, _('poemas más votados'));
		//$left_options[] = new MenuOption(_('más visitados'), $globals['base_url'].'top_visited', $id, _('poemas más visitados/leídos'));
		//$left_options[] = new MenuOption(_('destacados'), $globals['base_url'].'top_active', $id, _('poemas más activos'));
		$left_options[] = new MenuOption(_('subs'), $globals['base_url_general'].'subs', $id, _('sub poémames'));



		$right_options = array();
		//$right_options[] = new MenuOption(_('subs'), $globals['base_url_general'].'subs', $id, _('sub poémames'));
		...
		$right_options[] = new MenuOption(_('nuevos'), $globals['base_url'].'queue', '', _('votar poemas pendientes'));
...
function do_best_stories() {
	...
	$min_date = date("Y-m-d H:i:00", $globals['now'] - 12960000); // 36 hours
