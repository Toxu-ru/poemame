...
if (!isset($_REQUEST['id']) && $url_args[0] && !ctype_digit($url_args[0])) { // Compatibility with story.php?id=x and /story/x
	...
	if (! $link ) {
		do_error(_('poema no encontrado'), 404);
	}
} else {
...
$globals['extra_head'] .= '<link rel="alternate" type="application/rss+xml" title="'._('comentarios este poema').'" href="'.$globals['scheme'].'//'.get_server_name().$globals['base_url'].'comments_rss?id='.$link->id.'" />';
...
case 3:
	...
	if ($globals['link']->sent_date < $globals['now'] - 60*86400) { // older than 60 days
		echo _('Poema antiguo, datos de votos archivados');
	} else {
