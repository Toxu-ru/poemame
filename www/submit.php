...
if(isset($_POST["phase"])) {
	switch ($_POST["phase"]) {
		case 1:
			do_header(_('enviar poema') . " ", _('enviar poema'));
			if (! do_submit1()) {
			...
			break;
		case 2:
			do_header(_('revisar poema') . " ", _('revisar poema'));
			if (! do_submit2()) {
				// Just to display error messages
				$link = new Link;
				$link->randkey = rand(10000,10000000);
	...
	check_already_sent();
	do_header(_('enviar enlace') . " ", _('enviar enlace'));
	do_submit0();
}
...
function do_submit1() {
	...
	if ($anti_spam) {
		...
		if ($globals['min_karma_for_links'] > 0 && $current_user->user_karma < $globals['min_karma_for_links'] ) {
			add_submit_error( _('no tienes el mínimo de karma para enviar un nuevo poema'));
			return false;
		}
	...
	if ($new_user && $globals['min_user_votes'] > 0 && $current_user->user_karma < $globals['new_user_karma']) {
		...
		if (!$current_user->admin && $user_votes < $min_votes) {
			...
			if ($new_user) {
				add_submit_error( _('¿es la primera vez que envías un poema?'),
					_('necesitas como mínimo'). " $needed " . _('votos'));
			} else {
				add_submit_error( _('no tienes el mínimo de votos necesarios para enviar un nuevo poema'),
	...
	if ($anti_spam) {
		...
		if ($enqueued_last_minutes > $enqueued_limit) {
			add_submit_error( _('exceso de envíos'),
				_('se han enviado demasiados poemas en los últimos 3 minutos'). " ($enqueued_last_minutes > $enqueued_limit), "._('disculpa las molestias'));
...
function link_errors($link) {
	...
	if($link->status != 'discard') {
		add_submit_error(_("el poema ya está en cola").": $link->status");
		$error = true;
	}
