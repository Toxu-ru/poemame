...
if(!empty($_REQUEST['time'])) {
	...
	$user_login = $db->get_var("select user_login from users where user_id=$user_id");
	$title = $globals['site_name'].': '.sprintf(_('poemas de %s'), $user_login);
	$globals['redirect_feedburner'] = false;
} elseif (isset($_REQUEST['active'])) {
	$globals['redirect_feedburner'] = false;
	$title = $globals['site_name'].': '._('más activos');
	...
	*/
	...
	switch ($status) {
		...
		case 'all_local':
		default:
			$title = $globals['site_name'].': '._('todos');
			$order_field = 'date';
			$link_date = "date";
			break;
	}
...
if ($links) {
	foreach($links as $link_id) {
		...
		if (isset($_REQUEST['nohtml'])) {
			...
			echo " $rel>"._('poema original')."</a> (".parse_url($link->url, 1).")</p>";
