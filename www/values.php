...
do_header(_('Información sobre valores de karma y límites') . ' | ' . _('poémame'));
...
echo _("El karma de un usuario puede ir de").' '.$globals['min_karma'].' '._("a").' '.$globals['max_karma'].'<br/>
				<br/>
				'._("El karma base va de").' '.$globals['karma_base'] .' '._("a").' '.$globals['karma_base_max'] .'. '. _("Aumentando a una velocidad de 1 por año, a contar desde la fecha del primer poema publicada en portada de ese usuario") .'<br/>
				<br/>
...
if($globals['min_karma_for_links']) {
				echo _("Karma mínimo para enviar poemas") . ': ' . $globals['min_karma_for_links'] . '<br/>
				<br/>
				';
}
...
echo '
		<fieldset id="comments">
			<legend>'._('comentarios').'</legend>
				...
				<br/>
				'._("Tiempo que permanecen abiertos los comentarios en poemas en portada") . ': ' . print_time($globals['time_enabled_comments']) . '<br/>
				<br/>
				'._("Tiempo que permanecen abiertos los comentarios en poemas pendientes") . ': ' . print_time($globals['time_enabled_comments_status']['queued']) . '<br/>
				<br/>
				'._("Tiempo que permanecen abiertos los comentarios en poemas descartados") . ': ' . print_time($globals['time_enabled_comments_status']['discard']) . '<br/>
				<br/>
				'._("Tiempo que permanecen abiertos los comentarios en poemas autodescartados") . ': ' . print_time($globals['time_enabled_comments_status']['autodiscard']) . '<br/>
				<br/>
				'._("Tiempo que permanecen abiertos los comentarios en poemas descartados por abuso") . ': ' . print_time($globals['time_enabled_comments_status']['abuse']) . '<br/>
				<br/>
...
echo '
		<fieldset id="links">
			<legend>'._('envíos').'</legend>
				'._("Límite de envíos global para usuarios con karma") . ' &lt;= ' . $globals['limit_3_minutes_karma'] . ' ('. _('se han enviado demasiados poemas en los últimos 3 minutos') ."): " . $globals['limit_3_minutes'] . ' ' . _("votos cada 3 minutos") . '<br/>
				<br/>
				'._("Límite de envíos global para usuarios con karma") . ' > ' . $globals['limit_3_minutes_karma'] .' ('. _('se han enviado demasiados poemas en los últimos 3 minutos') ."): " . ($globals['limit_3_minutes'] * 1.5) . ' ' . _("votos cada 3 minutos") . '<br/>
				<br/>
				'._("Límite de envíos por usuario en las últimas 24 horas") . ' (' . _('debes esperar, tienes demasiados poemas en cola de las últimas 24 horas') . "): " .$globals['limit_user_24_hours'] . ' ' ._("envíos") .'<br/>
				<br/>
				...
				<br/>
				'._("Máximo de borradores por usuario") . ' (' . _('has hecho demasiados intentos, debes esperar o continuar con ellos desde la') . " " . _('cola de descartados') . '): ' . $globals['draft_limit'] . '<br/>
				<br/>
				'._("Porcentaje de karma de poema que se despublica y vuelve a pendientes"). ': ' . intval(100 / $globals['depublish_karma_divisor']) . '<br/>
		</fieldset>';
...
echo '
		<fieldset id="formulas">
			<legend>'._('fórmulas').'</legend>
				'._("La despublicación» ocurre cuando la suma del karma de los votantes de negativos > suma del karma de los votantes de positivos o que la suma de los votantes negativos > (karma del poema / 2)") . '<br/>
				<br/>
				'._("Para despublicar también debe cumplirse que la suma del karma de los votantes de negativos > (karma del envío / 6) o que el núm. de votos negativos > (núm. de votos positivos / 6)") . '<br/>
				<br/>
				'._("Para que sean contados en las sumas de karma del cálculo de una despublicación, los usuarios que voten positivo deben tener karma > ") . $globals['depublish_positive_karma'] . _(" y los que voten negativo karma > ") . $globals['depublish_negative_karma'] . '<br/>
				<br/>
				'._("Se considera «nuevo usuario» a los usuarios que no hayan enviado ningún voto o se hayan registrado hace menos de "). print_time($globals['new_user_time']) . '<br/>
				<br/>';
...
echo '
				...
				<br/>
				'._("Un «nuevo usuario» con karma &lt; "). $globals['new_user_karma'] . _(" y con envíos deberá votar (cifra dinámica) ") . min(4, intval($total_links/20)) . _(" * (1 + nº de envíos del usuario en las últimas 24 h. que no estén en estado «descartado») votos para poder enviar") . '<br/>
				<br/>';
}

echo '
				'._("Un «nuevo usuario» solo podrá enviar ") . $globals['new_user_links_limit'] . ' ' .  _("poemas cada") . ' ' .print_time($globals['new_user_links_interval']) . ' ('._('debes esperar, ya se enviaron varias con el mismo usuario o dirección IP'). ')<br/>
				<br/>
