...
function do_login() {
	...
	if (! $globals['mobile'] && $previous_login_failed < 3 && empty($_POST["processlogin"]) && empty($_COOKIE['u'])) {
		...
		if (!empty($_REQUEST['return']) && preg_match('/submit\.php/', $_REQUEST['return'])) {
			echo '<p style="border:1px solid #FF9400; font-size:1.3em; background:#FEFBEA; font-weight:bold; padding:0.5em 1em;">Para enviar un poema debes ser un usuario registrado</p>'."\n";
		}
		echo '<h3>'._('¿Qué es poémame?').'</h3>'."\n";
		echo '<p>'._('Es un sitio que te permite enviar tus poemas, que serán revisados por todos y serán promovidos, o no, a la página principal. Cuando un usuario envía un poema éste queda en la <a href="queue">cola de pendientes</a> hasta que reúne los votos suficientes para ser promovido a la página principal').'.</p>'."\n";

		echo '<h3>'._('¿Todavía no eres usuario de Poémame?').'</h3>'."\n";
		echo '<p>'._('Como usuario registrado podrás, entre otras cosas').':</p>'."\n";
		echo '<ul style="margin-left: 1.5em">'."\n";
		echo '<li>'."\n";
		echo '<strong>'._('Enviar poemas').'</strong><br />'."\n";
		echo '<p>'._('Una vez registrado puedes enviar los poemas que quierasc compartir con la comunidad. Si tienes algún tipo de duda sobre que tipo de poemas puedes enviar revisa nuestras <a href="faq-es.php">preguntas frecuentes sobre Poémame</a>').'.</p>'."\n";
		echo '</li>'."\n";
		echo '<li>'."\n";
		echo '<strong>'._('Escribir comentarios').'</strong><br />'."\n";
		echo '<p>'._('Puedes escribir tu opinión sobre los poemas enviados a oémame mediante comentarios de texto. También puedes votar positivamente aquellos comentarios constructivos, ingeniosos o interesantes y negativamente aquellos que consideres inoportunos').'.</p>'."\n";
		echo '</li>'."\n";
		echo '<li>'."\n";
		echo '<strong>'._('Perfil de usuario').'</strong><br />'."\n";
		echo '<p>'._('Toda tu información como usuario está disponible desde la página de tu perfil. También puedes subir una imagen que representará a tu usuario en poémame. Incluso es posible compartir los ingresos publicitarios de poémame, solo tienes que introducir el código de tu cuenta Google Adsense desde tu perfil').'.</p>'."\n";
		echo '</li>'."\n";
		echo '<li>'."\n";
		echo '<strong>'._('Chatear en tiempo real desde la fisgona').'</strong><br />'."\n";
		echo '<p>'._('Gracias a la <a href="sneak.php">fisgona</a> puedes ver en tiempo real toda la actividad de poémame. Además como usuario registrado podrás chatear con mucha más gente de la comunidad poémame').'</p>'."\n";
		echo '</li>'."\n";
		echo '</ul>'."\n";
